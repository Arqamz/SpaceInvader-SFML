...
public:
	...
	PowerUp(std::string texture_path) {
	...
	void dropAddOn() {
        ...
        // Generate random number between 800 and 1000 (like a delay)
        int lowerBound = std::rand() % 801 + (rand() % 801 + 1000);
...
// Useless polymorphism when its the same function for all of them?